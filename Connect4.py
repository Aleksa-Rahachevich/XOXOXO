...
class Game:
    ...
    def check_value(self, row, column):
        for i in range(max(row - 1, 0), min(min(row + 1, self.field.number_of_rows) + 1, self.field.number_of_rows)):
    ...
    def coordinate_selection(self, coordinates):
        ...
        self.player_row_number = coordinate[0]
        self.player_column_number = coordinate[1]
        return self.player_column_number, self.player_row_number
